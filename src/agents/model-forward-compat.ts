...
// are not yet in pi-ai's built-in catalogs. Clone the gemini-3 templates so users
// don't get "Unknown model" errors when Google releases new models.
...
const GOOGLE_PROVIDER_VARIANTS = new Set([
  "google",
  "google-vertex",
  "google-gemini-cli",
  "google-antigravity",
...
// gemini-3.1-pro-preview / gemini-3.1-flash-preview / gemini-3.1-flash-lite-preview
// are not present in pi-ai's built-in catalogs yet. Clone the nearest gemini-3 template
// so users don't get "Unknown model" errors when Google releases new minor-version models.
// Supports all Google provider variants: google, google-vertex, google-gemini-cli,
// google-antigravity.
function resolveGoogle31ForwardCompatModel(
  ...
  const normalizedProvider = normalizeProviderId(provider);
  if (!GOOGLE_PROVIDER_VARIANTS.has(normalizedProvider)) {
    return undefined;
  }
  ...
  // Check flash-lite before flash to avoid prefix collision
  // ("gemini-3.1-flash-lite" starts with "gemini-3.1-flash")
  if (lower.startsWith(GEMINI_3_1_FLASH_LITE_PREFIX)) {
